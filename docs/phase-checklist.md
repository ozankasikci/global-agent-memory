...
| 10 — Daemon and stdio proxy | Complete | Authenticated localhost Streamable HTTP, SDK host/origin validation, request/connection limits, verified daemon lifecycle, protocol-pure stdio, cancellation preservation, explicit stdin-close exit, two-client shared state, stable unavailable errors, and CLI-over-MCP E2E tests pass. |
...
| 12 — Watcher and recovery | Complete | Persisted debounced jobs, startup reconciliation, watcher-driven external edits, bounded index/embedding retries including idle provider recovery, invalid/duplicate isolation, corruption quarantine, Markdown rebuild, and signal-driven daemon shutdown tests pass. |
| 13 — CLI, packaging, services, doctor | Complete | Complete MCP-routed runtime/project CLI, serve/daemon/MCP/config/integration namespaces, doctor, shell completion, safe backup/restore, pip/uv upgrade and rollback, natively enabled managed launchd/systemd services, upgrade fixture, subprocess lifecycle, and fresh-wheel tests pass. |
...
| 15 — Claude Code integration | Complete | Fake-home symlink/copy install, official user-scope CLI registration adapter, guarded JSON fallback, transactional manifest/backups/snippet rollback, status/verify/uninstall, conflict/idempotency, cleanup-safe shared-daemon verification pass; real user config was not mutated. |
| 16 — Codex integration | Complete | Fake-home symlink/copy install, official CLI registration adapter, marked TOML fallback, transactional manifest/backups/snippet rollback, status/verify/uninstall, conflict/idempotency, cleanup-safe shared-daemon verification pass; local Codex binary is broken so live acceptance is deferred. |
